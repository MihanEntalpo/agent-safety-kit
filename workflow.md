...
- Все команды подключения (`run`, `install-agents`, `shell`) используют `multipass ssh` с флагами `-L/-R/-D`, собранными из конфигурации.
...
### Проблемы и blockers
- Требовалось запускать multipass ssh/transfer через socks5-прокси, не ломая сценарии без прокси и позволяя временно менять настройку.
### Решения и заметки
