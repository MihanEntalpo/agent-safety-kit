...
## Задача: Обновить версию и changelog для addmount
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- Версия проекта повышена до 0.9.10, в changelog добавлено упоминание команды addmount.

## Задача: Добавить команду removemount для удаления монтирований
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- Реализована команда `removemount` с выбором монтирования, подтверждением, резервной копией конфига и опциональным `--umount`.
- Документация и локализации обновлены, добавлены тесты для удаления монтирований из YAML.

## Задача: Гарантировать удаление монтирования только после успешного umount
### Проблемы и blockers
- Требовалось убрать флаг `--umount` и обеспечить отказ от записи в конфиг при ошибке unmount.
### Решения и заметки
- Команда `removemount` теперь всегда выполняет `umount` перед записью конфига и не удаляет запись при ошибке.
- Документация и тесты обновлены, чтобы отразить обязательное размонтирование.

## Задача: Увеличить версию и описать removemount
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- Версия проекта повышена до 0.9.11, в changelog отражена команда removemount.

## Задача: Добавить opt-in инфраструктуру для host-интеграционных тестов
### Проблемы и blockers
- Требуется безопасный запуск тестов, которые модифицируют систему хоста (snapd/Multipass).
### Решения и заметки
- Добавлены маркеры pytest для host-интеграционных тестов и пропуск по умолчанию без явного `AGSEKIT_RUN_HOST_IT=1`.
- Введён общий guard на Linux-хосты и описан маркер для предотвращения случайного запуска.

## Задача: Добавить host-интеграционный тест для agsekit prepare
### Проблемы и blockers
- Тест должен удалять snapd перед запуском и требует sudo/apt на Debian-подобных системах.
### Решения и заметки
- Добавлен тест, который удаляет snapd при наличии, запускает `agsekit prepare`, а затем проверяет установку Multipass, доступность snap и генерацию SSH-ключей.

## Задача: Зафиксировать установку зависимостей перед запуском pytest
### Проблемы и blockers
- Тесты падают на импортах без установки зависимостей из `pyproject.toml`.
### Решения и заметки
- В `AGENTS.md` добавлена инструкция устанавливать зависимости перед запуском `pytest`.

## Задача: Добавить команду pip-upgrade для самообновления agsekit
### Проблемы и blockers
- Требовалось обновлять пакет только в том окружении, где запущена CLI, и корректно сообщать об отсутствии установки через pip.
### Решения и заметки
- Добавлена команда `pip-upgrade`, которая определяет активное Python-окружение, проверяет наличие agsekit через pip и запускает `pip install agsekit --upgrade`.
- Документация обновлена, версия повышена, а в changelog отмечена функция самообновления.

## Задача: Продолжать настройку при несовпадении ресурсов и улучшить ошибку run
### Проблемы и blockers
- Несовпадение параметров ВМ останавливало create-vm/create-vms до подготовки ключей и пакетов.
- Запуск несуществующего агента приводил к стек-трейсу вместо понятного сообщения.
### Решения и заметки
- Логика создания ВМ теперь продолжает подготовку и выводит предупреждение о несовпадении ресурсов после шагов настройки.
- Для команды run добавлено понятное сообщение о неизвестном агенте со списком доступных.

## Задача: Добавить ripgrep в подготовку ВМ при create-vm
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- В установку пакетов для ВМ добавлен ripgrep, чтобы инструмент был доступен после create-vm/create-vms.

## Задача: Добавить установку комплектов ПО для ВМ через поле install
### Проблемы и blockers
- Требовалось описать наборы ПО с зависимостями и обеспечить идемпотентную установку через bash-скрипты.
### Решения и заметки
- В конфигурацию ВМ добавлено поле install с поддержкой версий через суффикс `:` и валидацией в конфиге.
- Создан реестр комплектов и набор скриптов (pyenv, nvm, python, nodejs, rust, golang), которые выполняются при create-vm/create-vms.
- Документация и пример конфига обновлены описанием доступных комплектов и примерами использования.

## Задача: Добавить list-bundles и новый комплект docker
### Проблемы и blockers
- Нужно было вынести описание комплектов в отдельный каталог и синхронизировать документацию.
### Решения и заметки
- Добавлена команда list-bundles и реестр комплектов с описаниями; документация обновлена, включая комментарии.
- Добавлен комплект docker с установкой Docker Engine и Compose, а также соответствующий установочный скрипт.

## Задача: Добавить тесты локализации и зафиксировать запуск тестов
### Проблемы и blockers
- Обнаружена ошибка в JSON локализации (пропущенная запятая), из-за которой загрузка переводов могла падать.
### Решения и заметки
- Добавлены тесты для нормализации языка, определения языка окружения, fallback на английский и поведения при неизвестных ключах.
- Исправлен JSON английской локали и добавлена инструкция запускать тесты после каждого изменения.

## Задача: Исправить интерактивный выбор ВМ при create-vm
### Проблемы и blockers
- Интерактивный режим падал с TypeError из-за использования аргумента name в questionary.Choice.
### Решения и заметки
- Обновлено создание списка ВМ для create-vm на позиционный аргумент Choice, совместимый с текущей версией questionary.

## Задача: Уточнить правило запуска VM-интеграционных тестов
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- В `AGENTS.md` добавлено правило не запускать VM-интеграционные автотесты в облачном окружении, оставив их доступными для codex-cli.

## Задача: Вернуть запуск агентов через multipass exec и убрать socks5_proxy у агентов
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- Восстановлен запуск команд через `multipass exec` с proxychains внутри ВМ и удалена настройка `socks5_proxy` у агентов в конфиге, генераторе и тестах.

## Задача: Добавить команду backup-clean для удаления старых бэкапов
### Проблемы и blockers
- Для метода thin пока нет реализации, нужно временно выводить отдельное сообщение.
### Решения и заметки
- Добавлена команда `backup-clean`, которая ищет монтирование по `source`, удаляет старые снапшоты методом `tail` и сообщает о неготовности метода `thin`.
- Документация и локализации обновлены, добавлены тесты на очистку и обработку ошибок.

## Задача: Добавить очистку старых бэкапов и настройки retention для mount
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- Добавлена автоматическая очистка старых снапшотов после каждого бэкапа в командах повторных бэкапов и run.
- В конфигурацию mount добавлены параметры max_backups и backup_clean_method с поддержкой в config-gen, addmount и документации.

## Задача: Реализовать thin-очистку бэкапов
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- Реализован алгоритм логарифмического прореживания с защитой трёх последних бэкапов и учётом интервалов.
- Обновлены тесты и документация для описания методов tail и thin.

## Задача: Добавить backup-clean в интерактивное меню и переключить метод очистки на thin
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- Команда `backup-clean` добавлена в интерактивное меню и получила отдельные вопросы для выбора монтирования, количества копий и метода очистки.
- Метод очистки по умолчанию переключён на `thin` для монтирований, config-gen, addmount и самой команды `backup-clean`, документация обновлена.
- Очистка бэкапов теперь логирует удаляемые снапшоты: в `backup-clean` сообщения выводятся во время удаления, а в автоматической очистке — после выполнения.

## Задача: Поднять версию до 1.0.1 и зафиксировать изменения backup-clean
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- Версия проекта повышена до 1.0.1, в changelog отмечено добавление интерактивного backup-clean и переключение метода очистки по умолчанию на thin.

## Задача: Добавить команду version для отображения версии agsekit
### Проблемы и blockers
- Нужно получить версию установленного пакета и версию проекта в режиме разработки.
### Решения и заметки
- Добавлена команда `agsekit version`, которая выводит версию установленного пакета и берёт версию проекта из `pyproject.toml`, если файл доступен.
- Документация обновлена, чтобы описать новую команду.

## Задача: Добавить тесты для команды version и поднять версию релиза
### Проблемы и blockers
- Блокирующих проблем не выявлено.
### Решения и заметки
- Добавлены тесты для вывода версии установленного пакета и версии проекта.
- Версия проекта повышена на 0.0.1, а в changelog отмечено добавление команды `agsekit version`.

## Задача: Добавить fallback-зависимость для tomllib в команде version
### Проблемы и blockers
...
## Задача: Доработать плейбук установки codex-glibc очисткой rust/cargo после успешной проверки бинарника
### Проблемы и blockers
- Нужно было гарантировать, что очистка запускается только после подтверждения работоспособности установленного бинарника, чтобы не удалять toolchain раньше времени.
### Решения и заметки
- В `ansible/agents/codex-glibc.yml` добавлены обязательные шаги проверки существования `codex-glibc` и выполнения `codex-glibc --version`.
- После успешной проверки добавлен шаг очистки `~/.cargo` и `~/.rustup` (включая `rustup self uninstall -y`), чтобы освободить место после сборки.
- Версия проекта повышена на `0.0.1` до `1.0.3`, а функциональное изменение зафиксировано в `changelog.md`.


## Задача: Изменить `agsekit mount` на позиционный путь вместо `--source-dir` с авто-резолвом
### Проблемы и blockers
- Текущая команда `mount` принимала только флаг `--source-dir` и умела находить монтирование только по точному совпадению `source`, из-за чего не работали сценарии запуска из вложенных директорий (`.`/`../abc`).
- Нужна была обратная совместимость логики `umount`, где `--source-dir` уже используется и должен остаться без изменений.
### Решения и заметки
- В `mount` введён позиционный аргумент `source_dir` (без флага), сохранив `--all` и прежнюю схему выбора mount из конфигурации.
- Добавлен поиск mount по пути с поддержкой вложенных/относительных путей: выбирается наиболее специфичный (`longest prefix`) `source` из config.
- При неоднозначном совпадении одинаковой глубины добавлена явная ошибка конфигурации `mounts.source_ambiguous`.
- Добавлены тесты на сценарии: прямой путь, `.` внутри source, вложенный путь и `../abc` из соседней директории.

## Задача: Сделать `agsekit umount` с позиционным путём и авто-резолвом как у `mount`
### Проблемы и blockers
- После предыдущего изменения `mount` уже поддерживал позиционный путь, но `umount` оставался на `--source-dir`, из-за чего UX команд различался.
- Текст подсказок/ошибок в локалях всё ещё ссылался на `--source-dir` как обязательный селектор.
### Решения и заметки
- `umount` переведён на позиционный аргумент `source_dir` (аналогично `mount`) с сохранением `--all`.
- Логика выбора mount осталась общей (`_select_mounts`) и уже поддерживает резолв относительных и вложенных путей через `find_mount_by_path`.
- Обновлены локализованные сообщения конфликтов/подсказок, чтобы они ссылались на «путь источника или --all» вместо `--source-dir`.
- Добавлены тесты для `umount`: выбор по явному пути источника и резолв по вложенному пути.

## Задача: Увеличить версию на 0.0.1 и зафиксировать упрощение сигнатуры mount/umount
### Проблемы и blockers
- Нужно было корректно отразить именно функциональные изменения в `changelog.md`, без документационных правок.
### Решения и заметки
- Версия в `pyproject.toml` увеличена с `1.0.3` до `1.0.4` (+0.0.1).
- В `changelog.md` добавлена запись `1.0.4` про упрощение сигнатуры `mount/umount` с позиционным путём и авто-резолвом source.


## Задача: Убрать требование ручного даунгрейда ansible через фиксацию версии в зависимостях пакета
### Проблемы и blockers
- Ранее для обхода несовместимости использовалась ранняя проверка версии ansible-core с остановкой установки, что всё равно требовало ручного даунгрейда у пользователя.
### Решения и заметки
- Ограничена зависимость `ansible-core` в `pyproject.toml` до совместимого диапазона `>=2.16,<2.19` для Python 3.10+.
- Удалена прикладная проверка версии из `agsekit_cli/ansible_utils.py` и связанные локализации/тесты, чтобы опираться на pin зависимостей пакета.


## Задача: Увеличить версию на 0.0.1 после фиксации совместимости ansible-core
### Проблемы и blockers
- Требовалось формально выпустить изменение с фиксацией зависимости `ansible-core` в совместимом диапазоне.
### Решения и заметки
- Версия в `pyproject.toml` увеличена с `1.0.4` до `1.0.5` (+0.0.1).
- В `changelog.md` добавлена запись `1.0.5` о фиксации диапазона `ansible-core` для совместимости установки агентов.
