...
@debug_option
def status_command(config_path: Optional[str], debug: bool, non_interactive: bool) -> None:
        ...
        for vm_name, vm in vms.items():
            ...
            if not vm_mounts:
                ...
                for mount in vm_mounts:
                    ...
                    backups_flag = click.style(tr("status.yes"), fg="green") if backups_active else click.style(
                        tr("status.no"), fg="bright_black"
                    )
                    rows.append(
                        [
                            str(mount.source),
