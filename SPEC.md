...
`Agent Safety Kit` — это CLI-инструмент для безопасной работы с консольными AI-агентами (`qwen`, `codex`, `claude-code`, `codex-glibc`) через изоляцию в Multipass VM и регулярные инкрементальные бэкапы на хосте.
...
- `allowed_agents` (optional) — список имён агентов из секции `agents`.
...
- `allowed_agents` валидируется как список непустых строк; каждое имя должно существовать в `agents`.

### 6.5 Секция `agents`
Поля:
- `type` (обязателен) — тип агента: `qwen`, `codex`, `codex-glibc`, `claude-code`; alias `claude -> claude-code`.
...
2. определяет рабочую директорию в VM (на основе mount + относительного подпути);
3. проверяет ограничение `mount.allowed_agents` (если задано);
4. проверяет наличие бинарника агента в VM;
5. (если backups enabled) делает initial backup при необходимости;
6. запускает фоновый `backup-repeated` и пишет лог в `backup.log`;
7. запускает агента через `multipass exec ... bash -lc`;
8. по завершении агента останавливает backup-процесс.
...
- проверка применяется для запуска по `source` и любым подпапкам этого `source`;
...
- `claude-code.yml`: установка через официальный install script.
