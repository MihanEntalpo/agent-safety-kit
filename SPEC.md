...
### 8.6 Установка и запуск агентов
...
3. проверяет наличие бинарника агента в VM;
4. (если backups enabled) делает initial backup при необходимости;
5. запускает фоновый `backup-repeated` и пишет лог в `backup.log`;
6. запускает агента через `multipass exec ... bash -lc`;
7. по завершении агента останавливает backup-процесс.

### 8.7 Операционный доступ и сервисный режим
...
- Исторические шаги в `workflow.md` могут противоречить друг другу; источником истины для текущего поведения является код и тесты.
