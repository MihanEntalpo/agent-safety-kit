...
6. проверяет наличие бинарника агента в VM;
7. (если backups enabled) делает initial backup при необходимости;
8. запускает фоновый `backup-repeated` и пишет лог в `backup.log`;
9. запускает агента через `multipass exec ... bash -lc`;
10. по завершении агента останавливает backup-процесс.
