...
- добавляет локальные дату и время к каждой debug-записи;
...
9. в обычном режиме (без `--debug`) в интерактивном терминале показывает `dots`-spinner для pre-launch шагов (`mount` checks, временный workdir, pre-run backup, запуск фоновых backup'ов);
   - spinner останавливается перед prompts/обычным текстовым выводом и перед передачей терминала интерактивной сессии агента;
10. backup-логика перед стартом агента:
...
- `codex-glibc.yml`: установка `bubblewrap`, сборка из исходников `openai/codex` по точному Git tag `rust-v<version>`, локальное повышение rustc query recursion limit для upstream crate `codex-exec`, управление swap при нехватке памяти, установка бинарника `codex-glibc`, post-build проверка.
...
На хосте:
- installer `scripts/install/install.sh` создаёт per-user venv в `~/.local/share/agsekit/venv`, symlink `~/.local/bin/agsekit`, а при необходимости добавляет `export PATH="$HOME/.local/bin:$PATH"` в shell startup files; если `python3`/`python` не найден, скрипт спрашивает подтверждение и пытается поставить Python автоматически: на Debian/Ubuntu через `apt-get install -y --reinstall python3 python3-pip python3-venv`, на Arch через `pacman -Sy --needed --noconfirm python python-pip`, на macOS через `brew install python`, а при отсутствии Homebrew печатает ссылку на `https://brew.sh/` и завершает работу; если на Linux сам Python уже есть, но создание venv падает из-за отсутствующих `pip`/`venv` компонентов, скрипт автоматически доставляет их через пакетный менеджер и повторяет создание venv;
- Windows installer `scripts/install/install.ps1` создаёт per-user venv в `%USERPROFILE%\.local\share\agsekit\venv`, wrapper `%USERPROFILE%\.local\bin\agsekit.cmd`, а при необходимости добавляет `%USERPROFILE%\.local\bin` в пользовательский `PATH`; после изменения пользовательского `PATH` установщик обновляет `PATH` текущей PowerShell-сессии из Machine+User PATH, чтобы не терять стандартные entries вроде WindowsApps/`winget`; если Python 3.9+ не найден, скрипт спрашивает подтверждение и пытается установить его автоматически: сначала через `winget install --id Python.Python.3 --exact --source winget --scope user`, а если `winget` недоступен, скачивает с `https://www.python.org/downloads/windows/` самый свежий официальный installer под архитектуру хоста и запускает его в quiet-режиме;
