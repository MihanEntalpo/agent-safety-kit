...
from agsekit_cli.commands.stop import stop_command
...
def test_stop_single_vm(monkeypatch, tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(stop_command, ["agent", "--config", str(config_path)])
...
def test_stop_all_vms(monkeypatch, tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(stop_command, ["--all-vms", "--config", str(config_path)])
