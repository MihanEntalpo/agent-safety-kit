...
Каждый тип агента также умеет узнавать свою последнюю устанавливаемую upstream-версию. Суточная проверка запускается только для уникальных типов агентов из конфига и сохраняет результат вместе со временем обновления в `state.yaml`; VM при этом не опрашиваются. `install-agents --upgrade` использует этот кэш и не обращается к registry во время установки.
...
- `install-agents --upgrade` временно использует последнюю версию из кэша вместо pin из конфига и не изменяет сам конфиг.
- дополнительный `--force-check-versions` в режиме `--upgrade` немедленно обновляет выбранные типы из upstream и при ошибке проверки не откатывается к устаревшему кэшу.
