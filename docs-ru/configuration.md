...
* `agents.<agent_name>`
  ...
  * При запуске каждый профиль получает отдельный home внутри VM: `/home/ubuntu/.agent-homes/<agent_name>`
  * Поэтому config/auth/cache/state не смешиваются, даже если несколько профилей используют один `type`
...
* `agents.<agent_name>.env`
  ...
  * Эти значения применяются после автоматических `HOME`, `XDG_*` и agent-specific config variables от agsekit, поэтому могут их переопределить
