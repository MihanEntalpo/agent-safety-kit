...
global: 
  ...
  state_file: null
  # Частота перечитывания конфигурации worker-процессом portforward, по умолчанию каждые 10 секунд
...
agents:
  ...
  qwen:
    ...
    type: qwen
    # Необязательно: без поля используется последняя upstream-версия, stable — проверенная версия agsekit, также можно указать semver
    version: stable
...
* `global.state_file`
  ...
  * State хранит текущую и последнюю версии agsekit, а также последние устанавливаемые версии настроенных типов агентов вместе с UTC-временем их обновления
...
* `global.portforward_config_check_interval_sec`
  ...
  * Команда `agsekit portforward` выполняет проброс портов и динамически обновляет туннели при изменении конфигурации; демон agsekit сейчас управляет этой командой как одним из фоновых процессов
  * См. [Port Forwarding](networking.md#port-forwarding) и [portforward](commands/networking.md)
* `global.check_new_version`
  * Включает или отключает периодическую проверку новых версий agsekit и настроенных типов агентов
  * Используется командой `agsekit run` и фоновым демоном agsekit
  * По умолчанию `true`
* `global.check_new_version_interval_sec`
  * Как часто agsekit должен запускать фоновую проверку; для каждого успешно обновлённого типа агента отдельно действует фиксированный кэш на 24 часа
  * Используется командой `agsekit run` и фоновым демоном agsekit
  * По умолчанию `600`
...
- `agent_versions.<type>.latest_version` — последняя устанавливаемая версия уникального типа агента, присутствующего в конфиге
- `agent_versions.<type>.updated_at` — UTC timestamp успешного обновления этой версии
...
- операции read-modify-write защищены межпроцессным lock-файлом;
- изменения записываются через атомарную замену файла, поэтому параллельные процессы agsekit не теряют обновления друг друга.
* `vms` 
...
* `agents.<agent_name>.version`
  * Необязательная политика версии при установке
  * Если поле не задано, устанавливается последняя upstream-версия без фиксации
  * Значение `stable` выбирает проверенную версию из registry agsekit
  * Строка semver, например `1.2.3`, фиксирует агента на точной версии
