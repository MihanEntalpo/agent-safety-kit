...
* Нужен Python 3.9+. Если его нет, `install.sh` спрашивает подтверждение и пытается установить его автоматически.
* На Debian/Ubuntu `install.sh` ставит `python3`, `python3-pip` и `python3-venv`.
* На Arch `install.sh` ставит `python` и `python-pip`.
* Если Python на Linux уже есть, но нет поддержки `venv`, `install.sh` автоматически доставляет нужные пакеты `pip`/`venv` и повторяет создание окружения.
...
* Основной поддерживаемый сценарий - MacOs 13+, HomeBrew нужен для автоматической установки Python и для `agsekit prepare`
...
* Нужен Python 3.9+. Если его нет, `install.sh` спрашивает подтверждение и пытается поставить его через HomeBrew.
* Если HomeBrew отсутствует, `install.sh` выводит ссылку на официальную страницу (`https://brew.sh/`) и завершает работу.
...
* Нужен Python 3.9+. Если его нет, установщик спрашивает подтверждение и пытается установить его автоматически.
* Если доступен `winget`, `install.ps1` ставит Python через `winget`.
* Если `winget` недоступен, `install.ps1` скачивает с `python.org` самый свежий официальный Windows installer и запускает его.
...
* `install.ps1` может установить Python 3.9+ сам после подтверждения. При ручной установке Python нужно поставить отдельно.
