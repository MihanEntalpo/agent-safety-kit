...
Управлять фоновым демоном agsekit и запускаемыми им фоновыми процессами. Проброс портов — одна из функций демона, а не его назначение целиком.
...
В обоих случаях демон сохраняет абсолютный путь к текущему CLI `agsekit`. Его текущий worker portforward использует ту же установку и для дочерних `ssh`-процессов, не полагаясь на `PATH`.

Daemon backend также включает периодическую проверку новых версий: если включён `global.check_new_version`, фоновый сервис периодически запускает `agsekit check-new-version` с интервалом из `global.check_new_version_interval_sec`. Команда также обновляет просроченные cached-версии уникальных типов агентов из конфига; успешная запись каждого агента имеет TTL 24 часа.

Существующие systemd unit и launchd label сохраняют legacy-имена с `portforward` для совместимости. Эти идентификаторы не ограничивают демон только пробросом портов.
