...
Если `agsekit run` запущен из отсутствующей папки проекта и пользователь соглашается на временную workdir внутри VM, mount не выбирается и бэкапы для этой сессии не запускаются.
