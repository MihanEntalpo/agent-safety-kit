...
- контейнер даёт значительно менее слабую изоляцию от злонамеренного агента, наевшегося где-то prompt-инъекций. Из контейнера проще сбежать используя уязвимости ядра, нежели чем сбежать из ВМ.
...
1) Установка (вам понадобится Python 3.9+), поддерживается Deb/Arch linux, MacOs (с HomeBrew) и Windows (через WSL)
...
[Почитайте раздел Установка](./docs-ru/install.md)

2) Создание конфигурации через интерактивный "мастер настройки" 

```shell
agsekit config-gen
```

3) Первичная установка всего: multipass, создание виртуальной машины, установка агентов, установка пакетов ПО
